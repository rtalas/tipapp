...
/**
 * Get chat status for a league (enabled/suspended).
 * Used by the chat page to display appropriate UI.
 */
export async function getChatStatus(leagueId: number) {
  try {
    const session = await auth()
    if (!session?.user?.id) {
      return { success: false, error: 'Authentication required' }
    ...
    return {
      ...
      league: {
        ...
        suspendedAt: league.chatSuspendedAt,
      },
      leagueUserId: leagueUser.id,
    }
  } catch (error) {
    return {
      success: false,
      error: getErrorMessage(error, 'Failed to get chat status'),
    }
  }
}
