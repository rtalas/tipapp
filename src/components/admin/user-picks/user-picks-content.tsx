...
interface UserPicksContentProps {
  matches: MatchWithBets[]
  leagues: League[]
  users: User[]
}

export function UserPicksContent({ matches, leagues, users }: UserPicksContentProps) {
  ...
  const filteredMatches = useMemo(() => {
    return matches.filter((match) => {
      // League filter
      if (leagueFilter !== 'all' && match.leagueId !== parseInt(leagueFilter)) {
        return false
      }

      // Status filter
      if (statusFilter === 'evaluated' && !match.Match.isEvaluated) return false
      if (statusFilter === 'unevaluated' && match.Match.isEvaluated) return false

      // User filter (check if any bet belongs to user)
      if (userFilter !== 'all') {
        const hasUserBet = match.UserBet.some(
      ...
      if (search) {
        ...
        const homeTeam =
          match.Match.LeagueTeam_Match_homeTeamIdToLeagueTeam.Team.name.toLowerCase()
        const awayTeam =
          match.Match.LeagueTeam_Match_awayTeamIdToLeagueTeam.Team.name.toLowerCase()
        return `${homeTeam} ${awayTeam}`.includes(searchLower)
      }

      return true
    })
  }, [matches, search, leagueFilter, userFilter, statusFilter])
